...
const client = mqtt.connect(url, {
  clientId: `viewer-cam-${Math.random().toString(36).slice(2, 6)}`,
  protocolVersion: 4, reconnectPeriod: 0,
});
let n = 0;
client.on('connect', () => client.subscribe('sienovo/boats/+/camera', { qos: 0 }));
client.on('message', (t, p) => {
  n++;
  if (n <= 3) {
    const o = JSON.parse(p.toString());
    console.log(`[${n}]`, t, 'frame bytes=', o.frame.length, 'prefix=', o.frame.slice(0, 30));
  }
});
setTimeout(() => {
  console.log(`got ${n} camera frames in 4s`);
  process.exit(0);
}, 4000);
